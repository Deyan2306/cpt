...
void travel(unordered_map<int, vector<is>>& towns, int current_index, int planes_limit, int searched_index, int used_cars, int used_planes, int distance, int & max_distance, bool & been_to_the_end_flag) {
    ...
    for (size_t index = 0; index < towns[current_index].size(); index++) {
        ...
        if (towns[current_index][index].second == 1) {
            travel(towns, towns[current_index][index].first, planes_limit, searched_index, used_cars + 1, used_planes, distance + 1, max_distance, been_to_the_end_flag);
        } else if (towns[current_index][index].second == 2) {
            travel(towns, towns[current_index][index].first, planes_limit, searched_index, used_cars, used_planes + 1, distance + 1, max_distance, been_to_the_end_flag);
        }
    }
}
...
int main(void) {
    ...
    // > [A]:[(B, type),(C, type)]
    unordered_map<int, vector<is>> towns;   
    ...
    for (size_t index = 0; index < number_of_vehicles; index++) {
        ...
        towns[townA].push_back(pair<int, short>(townB, type));
    }
